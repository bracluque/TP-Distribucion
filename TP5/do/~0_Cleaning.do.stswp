...
*	Set working directory
	...
	global cwd  	= "D:\UNLP_Maeco\Distribucion\TP-Distribucion\TP5" 
...
* Base Hogar:
import delimited "${data}/Base_ENESHogares.csv"
save "${data}/ENES_hogares.dta", replace

* Base Personas:
clear all
import delimited "${data}/Base_ENESPersonas.csv"
save "${data}/ENES_personas.dta", replace

*Número Identificador de Vivienda (nocues) y el Número Identificador de Hogar (nhog)
merge m:1 nocues nhog using "${data}/ENES_hogares.dta", assert(3) keep(3) nogen
save "${data}/ENES_f.dta", replace

clear all
use "${data}/ENES_f.dta", replace
